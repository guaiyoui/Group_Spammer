...
def loss_function_subgraph_regularization(model, modularity_matrix, x, edge_index, train_labels, selected_nodes):
    ...
    # # 计算带权重的 NLL Loss
    # loss_cls = F.nll_loss(output, train_labels, weight=class_weights)
...
def train_GCN(model, modularity_matrix, adj, selected_nodes, val_nodes,
    ...
    for epoch in range(epochs):
        ...
        model.train()
        optimizer.zero_grad()
        output, b_recon = model(modularity_matrix, features, edge_index=adj)
...
def test_GCN(model, modularity_matrix, adj, features, test_mask, test_labels, all_test_idx, all_test_labels, save_name=None, dataset_name=None, sample_global=False):
    model.eval()
    output_all, b_recon = model(modularity_matrix, features, adj)
...
def get_modularity_matrix(graph):
    ...
    # Step 3: 计算总边数 m
    m = graph.size()  # 直接获取边的总数
    
    # Step 4: 计算 modularity matrix B
    n = len(degrees)
    B = np.zeros((n, n))
...
class run_wrapper():
    ...
    def run(self, strategy, num_labeled_list=[10, 15, 20, 25, 30, 35, 40, 50], max_budget=160, seed=1):
        ...
        model = get_model(args.model, self.features.size(1), self.features.size(0), 2, args.hidden, args.dropout,
                          args.cuda)
        ...
        for i in range(len(num_labeled_list)):
            ...
            if strategy == 'random':
                ...
                if args.model == 'GCN':
                    idx_train = query_uncertainty_GCN_ComGA(model, self.modularity_matrix, self.adj, self.features, budget, pool)
                else:
...
if __name__ == '__main__':
    ...
    if args.dataset == 'spammer':
        ...
        if args.test_percents in ['50percent', '30percent']:
