...
for i in range(5):
    ...
    # Load the datasets
    train_df = pd.read_csv('./50percent/'+training_file_name, header=None)
    test_df = pd.read_csv('./50percent/'+testing_file_name, header=None)

    # Randomly select 20% of the rows from train_1.csv
    train_update_df = train_df.sample(frac=0.9, random_state=42)

    # Save the selected 20% rows to train_1_update.csv
    train_update_df.to_csv('./45percent/'+training_file_name, index=False)
    ...
    test_update_df.to_csv('./45percent/' + testing_file_name, index=False, lineterminator='\n')