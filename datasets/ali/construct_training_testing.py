...
# 计算5%的数量
train_size = int(len(combined_df) * 0.1)
...
# 目标目录
output_dir = './Training_Testing/10percent'

# 创建目录（如果不存在）
os.makedirs(output_dir, exist_ok=True)

# 保存到新的CSV文件，不包含索引，使用空格作为分隔符
new_train_df.to_csv('./Training_Testing/10percent/train_4.csv', index=False, sep=' ', header=False)
new_test_df.to_csv('./Training_Testing/10percent/test_4.csv', index=False, sep=' ', header=False)
